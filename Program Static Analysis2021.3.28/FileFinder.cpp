...
void FileFinder::getCommonFilenames(vector<string>& common1, vector<string>& common2)
{
	...
	for (int j = 0; j < size1; j++)
	{
		...
		{
			string string1 = filenames1[j];
			string string2 = filenames2[m];
			if ((filenames1[j] == filenames2[m] || ( compare(filenames1[j], filenames2[m] )>= 0.94))&&(string1.find(tmp)!=string1.npos)&& (string2.find(tmp) != string2.npos))
			{
...
void getFiles(string path, vector<string>& files)
{
	...
	{
		string fullname = p.assign(path).append("\\").append(fileinfo.name);
		do
		{
			//如果是目录,迭代之
			//如果不是,加入列表
...
void getFilenames(vector<string>& files, string filenames[NUMBER_MAX])
{
	...
	{
		cout<<"当前正在处理的文件路径"<<files[j]<<endl;
		//files[j] = "D:\\test\\calculator-revert-758-Fix757\\NOTICE.txt";//测试出错的文件路径
